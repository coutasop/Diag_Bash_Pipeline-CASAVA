...
function usage
{
    ...
    echo "		 -casavaVariant <CASAVA variant Folder>"
		...
		-gatkDir | --gatkDir )         shift
					if [ "$1" != "" ]; then
						...
						exit
		                        ...
		                        ;;
		-casavaVariant | --casavaVariant )         shift
					if [ "$1" != "" ]; then
						casavaVariant=$1
					else
						usage
...
do
	echo -e "\n--------------------------------------------------------------------------------------------------------------"
	echo "$i";
	for j in `ls $casavaVariant/$i | grep Sample_`
	do
		echo "-----------------------"
		echo "$j"

		ind=$(echo $j | awk -F"_" '{print $2}')
		
		#execute les calculs profondeur uniquement si besoin (si les fichiers n'existe pas déjà)
		if [ -d $i/$j/genome/depth ]; then
			echo -e "\tOUTPUT FOLDER : $i/$j/genome/depth (folder created)"
			sudo chmod -R 777 $i/$j/genome/depth		#creation du repertoire depth s'il n'existe pas
		else
			mkdir $i/$j/genome/depth;					#s'il existe s'assurer des droits
		fi
		if [ -f $i/$j/genome/depth/depth.bed ]; then
